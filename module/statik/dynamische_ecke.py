...
class dynamische_ecke(ecke):
    ...
    def update(self, zukünftige_events, zeitänderung, fe_support):
        ...
        # Bewegung
        self.beschleunigung = self.wirkende_kraft(fe_support) / self.masse
        ...
        if np.linalg.norm(self.beschleunigung * self.masse) >= 1:
        ...
        versetzung = (self.beschleunigung * zeitänderung) * zeitänderung
        ...
        if (sum(self.beschleunigung**2) >= (0.05 / self.masse)**2).any() or (sum(self.geschwindigkeit**2) >= (0.05 / self.masse * zeitänderung)**2).any():
